...
\thusetup{
  ...
  %
  % title  = {分割-联邦学习(SplitFed)在标签噪声和非独立同分布数据下的性能评估与改进},
  title  = {面向异构数据与资源受限场景的分割-联邦学习协同优化研究},
  title* = {Research on Collaborative Optimization of Split Federated Learning for Heterogeneous Data and Resource-Constrained Scenarios},
...
\def\N{\mathcal{N}}
\def\D{\mathcal{D}}
\def\Dnoi{\mathcal{D}^{noi}}
\def\Dnoih{\hat{\mathcal{D}}^{noi}}
\def\Dcle{\mathcal{D}^{cle}}
\def\omegab{\boldsymbol{\omega}}
\def\omegabs{\omegab_{\text{s}}}
\def\omegabc{\omegab_{\text{c}}}
\def\omegabs{\omegab_{\text{s}}}
\def\omegabsn{\omegab_{\text{s},n}}
\def\omegabcn{\omegab_{\text{c},n}}
\def\zetanoi{\zeta^{noi}}
\def\zetacle{\zeta^{cle}}
\def\gb{\boldsymbol{g}}
\def \f{F}
\def\fcle{F^{cle}}
\def\fnoi{F^{noi}}
\def\epsilonb{\boldsymbol{\epsilon}}
\def\llocn{\boldsymbol{l}_{loc,n}}
% \usepackage{xcolor}
