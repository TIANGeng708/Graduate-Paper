...
针对这一缺失，本章提出一种新的分析路径：首先推导标签噪声对随机梯度的一阶矩（期望）与二阶矩（方差）的具体扰动，进而将其嵌入传统的收敛性证明框架以推导收敛速率。该策略不仅成功将标签噪声纳入 SFL 的理论分析体系，也为其他分布式学习算法的收敛性研究提供了通用的方法论参考。
...
\label{fig:compare-N}
\end{figure}
在SFL中，各类超参数都会对于模型的收敛产生影响，本部分实验目标是验证SFL在不同超参数下的收敛规律，包含SFL-V1和SFL-V2两个算法。

本地训练迭代次数 $E$ 是SFL中的关键超参数。通常情况下，$E$ 值过小会导致本地模型在未充分提取特征时即被聚合，
造成有效信息的丢失（欠拟合）；反之，$E$ 值过大则易引发本地过拟合，阻碍联邦服务器聚合出具有良好泛化性能的全局模型。
为此，本试验设定 $E \in \{2, 5, 10\}$ 开展对比实验。
由图 \ref{fig:compare-E} 所示，在 CIFAR-10 和 CIFAR-100 上，SFL-V1 和 SFL-V2 均呈现相同趋势：
$E$ 的变化几乎不改变最终准确率，但增大 $E$ 在加速收敛的同时，也加剧了准确率的震荡。
\begin{figure}[htbp]
\centering
...
参与训练的用户数量 $N$ 对模型的收敛过程也具有显著影响。
当 $N$ 较小时，单个用户的本地更新在全局聚合中占据较高权重，这虽有助于提升全局模型在参与用户上的泛化性能，
但也使得全局模型易受个别低质量本地模型的干扰，导致训练稳定性下降。反之，当 $N$ 较大时，单个用户的权重被稀释，
虽然降低了异常值对全局模型的负面影响，但也会削弱模型对特定用户分布的适应能力。
为此，本试验设定 $N\in\{10, 50, 100\}$ 进行对比实验。
如图 \ref{fig:compare-N} 所示，在 CIFAR-10 和 CIFAR-100 上，SFL-V1 和 SFL-V2 均呈现相同趋势：即$N$的变化几乎不改变模型最终准确率，但 $N$ 的增加会导致模型收敛速度放缓。


\subsection{Non-IID数据对模型收敛性的影响}\label{subsec:noniid-con}
...
本节实验专注于验证非独立同分布（Non-IID）数据对分割联邦学习（SFL）模型收敛性的影响。为了量化数据分布的不均匀程度，我们引入了狄利克雷分布参数 $\alpha$（在部分文献及本实验图示中记为 $\beta$），并设置了 $\alpha \in \{0.1, 0.5, 1, \infty\}$ 四种典型数据场景。其中，$\alpha \to \infty$ 代表理想的全局独立同分布（IID）状态，而 $\alpha = 0.1$ 则对应极度不平衡的 Non-IID 分布。我们在 CIFAR-10 和 CIFAR-100 数据集上分别考察了 SFL-V1 和 SFL-V2 两种架构的表现。

从理论层面分析，Non-IID 数据会导致各客户端本地模型产生显著的统计偏移（Statistical Shift），这种偏移破坏了全局模型聚合时的梯度一致性，进而阻碍收敛。图~\ref{fig:cifar10-noniid-con} 的实验结果有力地证实了这一假设，并揭示了 $\alpha$ 值对模型收敛性能的严格制约关系：

\begin{itemize}
    \item \textbf{收敛速率显著减缓：} 如图所示，随着 $\alpha$ 值的减小（从 $\infty$ 降至 $0.1$），模型达到稳定准确率所需的训练轮次（Training Rounds）明显增加。在 $\alpha=0.1$ 的极端场景下，曲线爬升缓慢，表明模型需要更多的通信迭代才能克服本地数据的偏差。
    ...
    \item \textbf{训练过程震荡加剧：} 观察训练轨迹可以发现，低 $\alpha$ 值对应的曲线波动幅度（Oscillation Amplitude）显著大于高 $\alpha$ 值场景。这是由于在高度 Non-IID 分布下，各客户端计算的梯度方向差异巨大，导致服务器端聚合后的全局模型更新方向不稳定，从而引发剧烈的准确率震荡。
\end{itemize}
...
{\includegraphics[width=0.35\linewidth]{/cifar100/sfl-v2/beta.pdf}}
\caption{Non-IID数据对模型准确率的影响}
\label{fig:cifar10-noise-con}
\end{figure}

\subsection{标签噪声对模型收敛性的影响}\label{subsec:noise-con}
...
\label{fig:cifar100-noise-con}
\end{figure}
在本试验中，我们旨在non-IID数据（$\alpha=0.1$）和IID数据（$\alpha=\infty$）下验证定理~\ref{theorem-nonconvex}--\ref{thm:cross-entropy}。为此，我们考虑两种情形：
(1)  60\% 的客户端存在标签噪声（部分含噪）；
(2) 所有客户端均存在标签噪声（全部含噪）。


图~\ref{fig:cifar10-noise-con} 与图~\ref{fig:cifar100-noise-con} 分别展示了 SFL-V2 在 
CIFAR-10 和 CIFAR-100 数据集上的训练轨迹。在各图中，不同曲线对应不同的噪声率。基于上述图表，我们得出以下几点观察：
首先，在大多数情况下，随着训练轮数的增加，准确率呈现出明显的拐点：在该点之前迅速上升，随后趋于平稳。
其次，非 IID 数据与标签噪声均会减缓模型收敛，表现为拐点的延迟出现。且噪声率越高，该效应愈发显著（例如，见图~\ref{fig:cifar100-noise-con3}）。这一现象与定理~\ref{theorem-nonconvex}--\ref{thm:cross-entropy} 中的理论分析相吻合。
第三，较高的标签噪声率会使模型更倾向于过拟合噪声标签（例如，见图~\ref{fig:cifar10-noise-con2}）。
第四，非 IID 数据与标签噪声均会显著降低最终模型的准确率。
